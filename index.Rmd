...
knitr::include_app("https://abbeyskinner.shinyapps.io/Time-Series/",
                   height = "500px")
```
...
```{r, fig.cap = "Summary of the gower_df data frame", out.width = "80%", fig.align = 'center'}
...
To visualize our clustering analysis results, we use the t-SNE(t-Distributed Stochastic Neighbor Embedding) technique, a dimension reduction technique that enables us to visualize a multi-dimensional data set in a two-dimensional plot.
...
```{r shiny-clustering1, eval = FALSE}
...
```{r shiny-clustering2, eval = FALSE}
