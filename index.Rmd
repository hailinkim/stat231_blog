...
The medioids table for 10 clusters grouped by race shows that uninsured respondents tend to be either American Indian and Alaska Native, Black, Asian, or Hispanic, except for cluster 7; possess less than a college degree except for cluster 9; their median income tends to be less than approximately $35K with the exception of White and Asian participants. Moreover, those with multiple social minority statuses are more likely to be not covered by health insurance than those with either the racial or educational aspect alone.
...
```{r}
set.seed(1291)
demographic_race <- demographic %>% 
  group_by(race, coverage) %>% 
  sample_n(75) %>% 
  ungroup() %>% 
  mutate(across(where(is.character), as.factor))
gower_df <- daisy(demographic_race, metric = "gower")
...
pam_nhis = pam(gower_df, diss = TRUE, k = 3)
